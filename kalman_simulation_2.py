...
class HybridAgent_opt_sim_probit:
    ...
    def get_probs(self, t):
        ...
        RU = (np.sqrt(sigma1) - np.sqrt(sigma2)) / std_dev
...
def estimate_h_i_and_Sigma_i(data, m, v):
    bounds = [(0.01, 7), (0.01, 7)]
    result = minimize(
    fun=neg_log_posterior,
    x0=np.random.uniform(low=[0.5, 0.5], high=[5.0, 5.0]),  # More dispersed initialization
    args=(data, m, v),
    bounds=bounds,
    method='Powell',
    options={
...
pd.concat(eb_histories, ignore_index=True).to_csv("data/eb_all_runs_truncated_prior_30EM5.csv", index=False)
df_results = pd.DataFrame(results)
df_results.to_csv("data/kalman_simulation_results_truncated_prior_30EM5.csv", index=False)
df_recovery = pd.DataFrame(recovery_records)
df_recovery.to_csv("data/part_param_recovery_results_truncated_prior_30EM5.csv", index=False)