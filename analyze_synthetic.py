...
print(f"seed for analysis is {SEED_FOR_ANALYSIS}")
...
if latent:
    ...
    latent_tensor = torch.load(f"data/latents_tensor{latent_nametag}{SEED_FOR_ANALYSIS}.pt")
    print(f"loaded latents_tensor{latent_nametag}.pt")
    print(f"dim of latent tensor: {latent_tensor.shape}")
else:
    latent_tensor = torch.load(f"data/latents_tensor{vanilla_nametag}{SEED_FOR_ANALYSIS}.pt")
...
y = 0.02 * np.random.randn(len(params))
plt.figure(figsize=(6, 2))
plt.scatter(params, y, c=params, cmap="coolwarm")
plt.yticks([])
plt.xlabel("alpha value")
plt.title("Original data generating process")
plt.savefig("plots/original_params_plotted.png", bbox_inches="tight")

# RSA_title = latent_nametag if latent else vanilla_nametag
# distance_matrix_params, plotting_order_params = plf.rsa_latents(latents = alphaP_test, metric="euclidean",title="test_params", reduction="entire", plot=True, original_data=True, cluster_order=True, linkage_method="average")
if latent:
    plf.plot_latents(dim_reduction="one_dim", latent_tensor = latent_tensor, avg=False, n_components=2, name=f"latent{SEED_FOR_ANALYSIS}", df=df_test, param_array = params)
    #distance_matrix_latents, plotting_order_latents = plf.rsa_latents(latents = latent_tensor, metric="euclidean",title=RSA_title, reduction="last", plot=True, original_data=False, cluster_order=True, linkage_method="average",fixed_order=plotting_order_params)

else:
    plf.plot_latents(dim_reduction="mds", latent_tensor = latent_tensor, avg=True, n_components=2, name=f"vanilla{SEED_FOR_ANALYSIS}", df=df_test, param_array = params)
    #distance_matrix_latents, plotting_order_latents = plf.rsa_latents(latents = latent_tensor, metric="euclidean",title=RSA_title, reduction="avg", plot=True, original_data=False, cluster_order=True, linkage_method="average",fixed_order=plotting_order_params)


# plot rsa of best model
...
distance_matrix_params, params_order = plf.rsa_latents(latents = params, metric="euclidean",title="test_params", reduction="entire", plot=False, original_data=True, cluster_order=False)
vec_params  = vectorize_rsa(distance_matrix_params)



rsa_dict = {}
base_dirs = ["runs", "runs_vanilla"]
for base_dir in base_dirs:
    correlation_gt = []
    with open(os.path.join(base_dir, "best_epoch_by_rsa.json"), "r") as f:
        meta = json.load(f)

    best_epoch_plot = meta["best_epoch"]
    seed_plot = 76 if base_dir == "runs" else meta["best_seed"] 
    #rsa_vecs_latents = [load_rsa_vector(seed, best_epoch_plot, base_dir) for seed in SEEDS]
    rsa_vec_latents = load_rsa_vector(seed_plot, best_epoch_plot, base_dir)

    dict_key = "IDRNN" if base_dir == "runs" else "vanilla"
    # for rsa_vec in rsa_vecs_latents:
    #     corr_matrix = np.corrcoef(vec_params,rsa_vec)[0,1]
    #     correlation_gt.append(corr_matrix)
    corr_matrix = np.corrcoef(vec_params,rsa_vec_latents)[0,1]
    #correlation_gt = np.array(correlation_gt)
    rsa_dict[dict_key] = {
    "mean": corr_matrix}

    # rsa_dict[dict_key] = {
    # "corrs": correlation_gt,
    # "mean": np.mean(correlation_gt),
    # "sem": sem(correlation_gt)}
...
means = [
    rsa_dict['IDRNN']['mean'],
    rsa_dict['vanilla']['mean']
]
# sems = [
#     rsa_dict['IDRNN']['sem'],
#     rsa_dict['vanilla']['sem']
# ]
...
ax.bar(
    ...
    means,
    #yerr=sems,
    #capsize=5,        # adds little caps to error bars
    color=bar_colors,
    alpha=0.8
)

ax.set_ylabel('Average correlation across seeds')
ax.set_title('Correlation between model latent geometry and ground truth')

plt.tight_layout()
plt.savefig("plots/bar_plot_simulation_rsa_corr_with_gt.png")
...
print(f"true model average loglik: {True_model_likelihood}")
...
violin_data = [
    Q_common_fit.values,
    Q_map.values,
    FQ_common_fit.values,
    FQ_map.values,
    rnn_common_process.values,
    rnn_ID.values,
    rnn_vanilla.values
]
#for barplot use ax.bar
ax.bar(models, likelihoods, label=bar_labels, color=bar_colors)
#for violinplot use ax.violinplot
#ax.violinplot(violin_data, showmeans=True, showextrema=True, widths=0.7)
#ax.boxplot(violin_data)
#labels only necessary if using violinplot
#ax.set_xticks(1, np.arange(len(models))+1)
#ax.set_xticklabels(models)

ax.axhline(True_model_likelihood, linestyle='--', color='black', linewidth=1.5, label='True model')
ax.set_ylim(bottom= True_model_likelihood-5)
ax.set_ylabel('mean log likelihood per participant')
ax.set_title('Performance across different Models')
ax.legend(title='Bar colors', loc='lower left')
# ---- Add significance lines ----
ymax = max(likelihoods)
